...
function TradingJournal({ quickAdd, onQuickAddDone }) {
  ...
  function addTrade() {
    if(!form.symbol||!form.entryPrice||!form.stopLoss||!form.tp1) return;
    const slPct  = ((parseFloat(form.stopLoss)-parseFloat(form.entryPrice))/parseFloat(form.entryPrice)*100*(form.direction==="long"?1:-1)*-1).toFixed(2);
    const tp1Pct = ((parseFloat(form.tp1)-parseFloat(form.entryPrice))/parseFloat(form.entryPrice)*100*(form.direction==="long"?1:-1)).toFixed(2);
    const tp2Pct = form.tp2?((parseFloat(form.tp2)-parseFloat(form.entryPrice))/parseFloat(form.entryPrice)*100*(form.direction==="long"?1:-1)).toFixed(2):"";
    const tp3Pct = form.tp3?((parseFloat(form.tp3)-parseFloat(form.entryPrice))/parseFloat(form.entryPrice)*100*(form.direction==="long"?1:-1)).toFixed(2):"";
    const newTrade = { ...form, id:Date.now(), slPct, tp1Pct, tp2Pct, tp3Pct, result:null, addedAt:new Date().toISOString() };
