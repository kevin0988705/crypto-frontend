        ...
        res.forEach(r=>{
          ...
          if(r.isEntry&&!seenAlerts.current.has(key+"e")){
            ...
            if(Notification.permission==="granted")
              new Notification(`🚀 進場：${r.symbol.replace("USDT","")}`,{body:`評分${r.score} · ${_iv}`,icon:"/icon-192.png"});
          }
          ...
          if((r.crash?.isBearSetup||r.crash?.isCrash)&&!seenAlerts.current.has(key+"b")){
            if(r.crash?.isBearSetup&&!r.crash?.isCrash){
              ...
              if(Notification.permission==="granted")
                new Notification(`⚠️ 準備下跌：${r.symbol.replace("USDT","")}`,{body:r.crash.crashReasons[0]||"",icon:"/icon-192.png"});
            }
          }
        });
