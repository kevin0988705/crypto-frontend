...
function AlertSidebar({alerts, onDismiss}) {
  ...
  const resultColor = r => r==="🚀 進場訊號"?"#4caf50":r==="🚀 暴漲預警"?"#ff9800":"#ef5350";

  return(
    <div style={{
      position:"fixed", top:0, right:0, width:320, height:"100vh",
      background:"#0a0f1e", borderLeft:"1px solid #1a2035",
      zIndex:800, display:"flex", flexDirection:"column",
      boxShadow:"-6px 0 32px #00000088", overflowY:"auto"
    }}>
      {/* Header */}
      <div style={{padding:"14px 16px", borderBottom:"1px solid #1a2035",
        display:"flex", justifyContent:"space-between", alignItems:"center",
        background:"#080c18", position:"sticky", top:0, zIndex:1}}>
        <div>
          <div style={{fontSize:9,color:"#ff9800",letterSpacing:3,marginBottom:2}}>LIVE ALERTS</div>
          <div style={{fontSize:14,fontWeight:800,color:"#e8eaf6"}}>即時訊號通知</div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:8,height:8,borderRadius:"50%",background:"#4caf50",
            boxShadow:"0 0 6px #4caf50",animation:"pulse 2s infinite"}}/>
          <button onClick={()=>onDismiss("all")}
            style={{background:"none",border:"1px solid #1a2035",color:"#546e7a",
              cursor:"pointer",fontSize:10,padding:"3px 8px",borderRadius:4,fontFamily:"inherit"}}>
            全部清除
          </button>
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"10px 12px",display:"flex",flexDirection:"column",gap:8}}>
        {alerts.map((a,i)=>{
          ...
          const bg = isCrash?"#1a050522":isSurge?"#1a100022":"#0d282022";
          ...
          return(
            <div key={a.symbol+i} style={{background:bg,border:`1px solid ${bc}44`,
              borderLeft:`3px solid ${bc}`, borderRadius:8, padding:"12px 12px",
              animation:"slideInRight .3s ease"}}>

              {/* Top row */}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div>
                  <div style={{fontSize:9,color:bc,letterSpacing:2,marginBottom:2,fontWeight:700}}>{title}</div>
                  <div style={{fontSize:18,fontWeight:800,color:"#e8eaf6",lineHeight:1}}>
                    {a.symbol.replace("USDT","")}
                    <span style={{fontSize:9,color:"#37474f",marginLeft:5}}>/USDT</span>
                  </div>
                </div>
                <button onClick={()=>onDismiss(i)}
                  style={{background:"none",border:"none",color:"#37474f",cursor:"pointer",fontSize:16}}>✕</button>
              </div>

              {/* Decision */}
              {dec&&(
                <div style={{background:"#060c1a",borderRadius:6,padding:"8px 10px",
                  border:`1px solid ${dec.decisionColor}44`,marginBottom:8}}>
                  <div style={{fontSize:16,fontWeight:900,color:dec.decisionColor,marginBottom:2}}>
                    {dec.decision}
                  </div>
                  <div style={{fontSize:10,color:"#546e7a"}}>{dec.urgency}</div>
                </div>
              )}
              ...
              <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                <span style={{fontSize:10,color:"#ff9800",background:"#ff950018",
                  padding:"2px 7px",borderRadius:4,fontWeight:700}}>評分 {a.score}</span>
                {a.vpt&&<span style={{fontSize:10,color:a.vpt.verdictColor,
                  background:a.vpt.verdictBg,padding:"2px 7px",borderRadius:4,fontWeight:700}}>
                  {a.vpt.verdictIcon} {a.vpt.verdict}</span>}
                {a.smc&&<span style={{fontSize:10,color:a.smc.smcColor,
                  background:`${a.smc.smcColor}18`,padding:"2px 7px",borderRadius:4}}>
              ...
              {!isSurge&&!isCrash&&a.trade&&(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",
                  ...
                  {[
                    {label:"📍 進場",val:a.trade.entryIdeal,col:"#e8eaf6"},
                    {label:"🛑 止損",val:`${a.trade.stopLoss} (${a.trade.slPct}%)`,col:"#ef5350"},
                    {label:"🎯 TP1",val:`${a.trade.tp1} (+${a.trade.tp1Pct}%)`,col:"#4caf50"},
                  ].map(item=>(
                    <div key={item.label} style={{background:"#060810",borderRadius:5,padding:"6px 7px",textAlign:"center"}}>
                      <div style={{fontSize:8,color:"#37474f",marginBottom:2}}>{item.label}</div>
                      <div style={{fontSize:10,fontWeight:700,color:item.col,lineHeight:1.3}}>{item.val}</div>
                    </div>
                  ))}
                </div>
              )}
              ...
              {reasons&&reasons.slice(0,3).map((r,j)=>(
                <div key={j} style={{fontSize:10,color:"#546e7a",marginBottom:2,display:"flex",gap:4}}>
                  <span style={{color:bc}}>{isCrash?"▼":"▲"}</span>{r}
                </div>
              ))}

              {/* Timestamp */}
              <div style={{fontSize:9,color:"#263238",marginTop:6}}>
  ...
  return(
    <div style={{position:"fixed",top:20,right:20,zIndex:9999,display:"flex",
      flexDirection:"column",gap:12,maxWidth:400}}>
      {alerts.map((a,i)=>{
        const isSurge=a.alertType==="surge",isCrash=a.alertType==="crash";
        const bc=isCrash?"#ef5350":isSurge?"#ff9800":"#4caf50";
        ...
        const dec=!isSurge&&!isCrash?calcEntryDecision(a):null;
        return(
          <div key={a.symbol+i} style={{background:bg,border:`2px solid ${bc}`,
            borderRadius:14,padding:"16px 18px",
            boxShadow:`0 10px 40px #00000099,0 0 24px ${bc}55`,animation:"slideIn .3s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div style={{flex:1}}>
                ...
                {dec&&(
                  <div style={{background:dec.decisionBg,borderRadius:10,padding:"10px 14px",
                    border:`1px solid ${dec.decisionColor}55`,marginBottom:10}}>
                    <div style={{fontSize:20,fontWeight:900,color:dec.decisionColor,marginBottom:4}}>
                      {dec.decision}
                    </div>
                ...
                <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:8}}>
                  <span style={{fontSize:13,color:"#ff9800",background:"#ff950022",
                    padding:"3px 9px",borderRadius:5,fontWeight:700}}>評分 {a.score}/100</span>
                  {a.smc&&<span style={{fontSize:13,color:a.smc.smcColor,
                    background:`${a.smc.smcColor}18`,padding:"3px 9px",borderRadius:5,fontWeight:700}}>
                    {a.smc.smcVerdict}</span>}
                  {a.vpt&&<span style={{fontSize:13,color:a.vpt.verdictColor,
                    background:a.vpt.verdictBg,padding:"3px 9px",borderRadius:5,fontWeight:700}}>
                    {a.vpt.verdictIcon} {a.vpt.verdict}</span>}
                </div>
                {!isSurge&&!isCrash&&a.trade&&(
                  <div style={{background:"#060c1a",borderRadius:8,padding:"10px 12px",
                    border:`1px solid ${bc}44`,fontSize:13}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",
                      fontSize:12,color:"#455a64",borderTop:"1px solid #1a2035",paddingTop:6}}>
              ...
              <button onClick={()=>onDismiss(i)}
                style={{background:"none",border:"none",color:"#37474f",
                  cursor:"pointer",fontSize:20,padding:"0 4px",marginLeft:8}}>✕</button>
            </div>
          </div>
        );
      })}
...
// 優先用 sessionStorage（Vercel 支援），fallback 用 in-memory
let _memJournal = [];
function loadJournal() {
  try {
    const raw = sessionStorage.getItem(JOURNAL_KEY);
    if(raw) { _memJournal = JSON.parse(raw); return _memJournal; }
  } catch(_) {}
  return _memJournal;
}
function saveJournal(trades) {
  _memJournal = trades;
  try { sessionStorage.setItem(JOURNAL_KEY, JSON.stringify(trades)); } catch(_) {}
}

async function checkTradeResult(trade) {
  // 抓取該幣種從進場時間到現在的K線，判斷是否觸發止盈或止損
  try {
    const res = await fetch(
      `${FAPI}/fapi/v1/klines?symbol=${trade.symbol}&interval=15m&limit=200`
    );
    ...
    const candles = klines
      .map(k=>({time:+k[0], high:+k[2], low:+k[3], close:+k[4]}))
      .filter(c => c.time >= entryTime);
    ...
    for(const c of candles) {
      ...
      const pnl = dir===1
        ? (c.close - entry)/entry*100
        : (entry - c.close)/entry*100;
...
function TradingJournal({ quickAdd, onQuickAddDone }) {
  ...
  // Use setTrades with callback to always read latest form state
  const handleAddTrade = () => {
    // Read directly from form ref to avoid stale closure
    setForm(currentForm => {
      if(!currentForm.symbol||!currentForm.entryPrice||!currentForm.stopLoss||!currentForm.tp1) {
        alert("請填入：進場價格、止損、TP1");
        return currentForm;
      }
      const ep  = parseFloat(currentForm.entryPrice);
      const sl  = parseFloat(currentForm.stopLoss);
      const t1  = parseFloat(currentForm.tp1);
      const t2  = parseFloat(currentForm.tp2||"0");
      const t3  = parseFloat(currentForm.tp3||"0");
      const dir = currentForm.direction==="long" ? 1 : -1;
      const slPct  = ((sl -ep)/ep*100*dir*-1).toFixed(2);
      const tp1Pct = ((t1 -ep)/ep*100*dir).toFixed(2);
      const tp2Pct = currentForm.tp2 ? ((t2-ep)/ep*100*dir).toFixed(2) : "";
      const tp3Pct = currentForm.tp3 ? ((t3-ep)/ep*100*dir).toFixed(2) : "";
      const newTrade = {
        ...currentForm,
        id: Date.now(),
      ...
      setShowForm(false);
      return { ...currentForm, entryPrice:"", stopLoss:"", tp1:"", tp2:"", tp3:"", note:"", score:"" };
    });
  };
  // Keep old addTrade name pointing to new handler
  ...
  async function checkResult(trade) {
    ...
    const updated = trades.map(t=>t.id===trade.id?{...t,result,checkedAt:new Date().toISOString()}:t);
    ...
    <div style={{display:"flex",flexDirection:"column",gap:3}}>
      ...
      <input type={type} value={form[key]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))}
        style={{background:"#060810",border:"1px solid #1a2035",borderRadius:5,
          color:"#90a4ae",fontSize:12,padding:"6px 8px",fontFamily:"inherit",outline:"none"}}/>
    </div>
  );
    ...
    <div style={{flex:1,overflowY:"auto",height:"100vh",background:"#07090f",padding:"16px 20px",boxSizing:"border-box"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <div style={{fontSize:20,fontWeight:800,background:"linear-gradient(90deg,#7986cb,#4caf50)",
            WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>交易日誌 · 自動回測</div>
          <div style={{fontSize:10,color:"#37474f",marginTop:2}}>記錄進場點位，系統自動回測是否觸發止盈止損</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={checkAll}
            style={{padding:"8px 16px",borderRadius:6,border:"1px solid #1a2035",
              background:"#0c111e",color:"#9fa8da",cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>
            🔄 全部回測
          </button>
        ...
        <div style={{background:"#0c111e",borderRadius:10,padding:"16px 18px",border:"1px solid #1a2035",marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:10}}>
              ...
              <div style={{display:"flex",gap:6}}>
                {["long","short"].map(d=>(
                  <button key={d} onClick={()=>setForm(f=>({...f,direction:d}))}
                    style={{flex:1,padding:"6px",borderRadius:5,cursor:"pointer",fontSize:12,fontFamily:"inherit",
            ...
            <div style={{display:"flex",flexDirection:"column",gap:3}}>
              ...
              <input type="datetime-local" value={form.entryTime} onChange={e=>setForm(f=>({...f,entryTime:e.target.value}))}
                style={{background:"#060810",border:"1px solid #1a2035",borderRadius:5,
                  color:"#90a4ae",fontSize:11,padding:"6px 8px",fontFamily:"inherit",outline:"none"}}/>
            </div>
            {inp("note","備注（訊號原因）")}
          </div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>setShowForm(false)}
              style={{padding:"9px 16px",borderRadius:6,border:"1px solid #1a2035",
                background:"transparent",color:"#546e7a",cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>
              取消
            </button>
          </div>
        </div>
      ...
      {trades.length===0?(
        ...
        <>
          ...
          <div style={{display:"grid",
            gridTemplateColumns:"90px 50px 80px 80px 70px 70px 70px 1fr 100px 100px 60px",
          ...
          {trades.map(t=>{
            ...
            return(
              <div key={t.id} style={{display:"grid",
                gridTemplateColumns:"90px 50px 80px 80px 70px 70px 70px 1fr 100px 100px 60px",
                gap:"0 8px",padding:"9px 8px",alignItems:"center",marginBottom:4,
                background:isWin?"#0d2820":isSL?"#1a0505":"#0c111e",
                borderRadius:6,
                border:`1px solid ${isWin?"#4caf5033":isSL?"#ef535033":"#111827"}`}}>
                ...
                <span style={{fontSize:12,fontWeight:700,color:t.direction==="long"?"#4caf50":"#ef5350"}}>
                  ...
                  {res&&(
                    <>
                      <div style={{fontSize:13,fontWeight:800,
                        color:parseFloat(res.currentPnl)>0?"#4caf50":parseFloat(res.currentPnl)<0?"#ef5350":"#90a4ae"}}>
                        {parseFloat(res.currentPnl)>0?"+":""}{res.currentPnl}%
                      </div>
                      <div style={{fontSize:9,color:"#37474f"}}>
                        最高+{res.maxProfit}% / 最低{res.maxLoss}%
                      </div>
                    </>
                  )}
                </div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:4}}>
                  <button onClick={()=>checkResult(t)} disabled={checking[t.id]}
                    style={{padding:"4px 8px",borderRadius:4,border:"1px solid #1a2035",
                      background:"#060c1a",color:"#7986cb",cursor:"pointer",fontSize:10,fontFamily:"inherit"}}>
                    {checking[t.id]?"...":"回測"}
                  </button>
...
async function fetchOIData(symbols) {
  const results = await Promise.allSettled(
    symbols.map(async sym => {
      ...
      // OI × 價格背離
      // 價漲OI降 = 空頭回補（不持久）; 價跌OI升 = 新空單進場（危險）
      // 價漲OI升 = 多頭增加（健康上漲）; 價跌OI降 = 多頭止損（底部）
      let divergence = "neutral", divColor = "#90a4ae", divNote = "";
      if(change24>1 && oiChg4h>5){
        divergence="bullAccum"; divColor="#4caf50";
        divNote="價漲OI升，多頭積累（健康）";
      } else if(change24>1 && oiChg4h<-5){
        divergence="shortCover"; divColor="#ff9800";
        divNote="價漲OI降，空頭回補（拉升可能減弱）";
      } else if(change24<-1 && oiChg4h>5){
        divergence="bearAccum"; divColor="#ef5350";
        divNote="價跌OI升，新空單進場（繼續下跌風險）";
      } else if(change24<-1 && oiChg4h<-5){
        divergence="longSqueezeEnd"; divColor="#ffb74d";
        divNote="價跌OI降，多頭止損出場（可能近底）";
      } else if(Math.abs(change24)<0.5 && Math.abs(oiChg4h)>8){
        divergence="coil"; divColor="#7986cb";
        divNote="價格橫盤但OI急變，方向即將決定🔥";
      }

      // 軋倉風險評估
      const frExtreme = frNow!=null&&Math.abs(frNow)>0.08;
      const oiHigh    = oiNow*price > 5e8; // OI值 > 5億USDT
    ...
    <div style={{flex:1,overflowY:"auto",height:"100vh",background:"#07090f",padding:"14px 18px",boxSizing:"border-box"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
        <div>
          ...
          <div style={{fontSize:18,fontWeight:800,background:"linear-gradient(90deg,#ff9800,#ef5350)",
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
        {[
          ...
          <div key={card.key}
            onClick={()=>setFilterDiv(filterDiv===card.key?"all":card.key)}
            style={{background:filterDiv===card.key?`${card.col}18`:"#0c111e",
              borderRadius:8,padding:"12px 14px",cursor:"pointer",
              border:`1px solid ${filterDiv===card.key?card.col:"#1a2035"}`,
              boxShadow:filterDiv===card.key?`0 0 12px ${card.col}33`:"none"}}>
        ...
        {[["oiChg1h","OI 1H變化"],["oiChg4h","OI 4H變化"],["fr","資金費率"],["vol","交易量"]].map(([k,l])=>(
          <button key={k} onClick={()=>setSortBy(k)}
            style={{padding:"4px 10px",borderRadius:4,fontSize:10,fontFamily:"inherit",cursor:"pointer",
              border:`1px solid ${sortBy===k?"#ff9800":"#1a2035"}`,
              background:sortBy===k?"#ff980022":"transparent",
              color:sortBy===k?"#ff9800":"#37474f"}}>
            {l}
          </button>
        ))}
        {filterDiv!=="all"&&(
          <button onClick={()=>setFilterDiv("all")}
            style={{padding:"4px 10px",borderRadius:4,fontSize:10,fontFamily:"inherit",cursor:"pointer",
              border:"1px solid #37474f",background:"transparent",color:"#546e7a"}}>
            ✕ 清除篩選
          </button>
        )}
      </div>
      ...
      {/* 表頭 */}
      <div style={{display:"grid",
        gridTemplateColumns:"100px 80px 70px 70px 70px 80px 90px 80px 1fr",
        gap:"0 8px",padding:"0 8px 6px",fontSize:9,color:"#263238",
        letterSpacing:1,textTransform:"uppercase",borderBottom:"1px solid #0f1629",marginBottom:6}}>
      ...
      {loading&&oiData.length===0&&(
        <div style={{textAlign:"center",color:"#37474f",marginTop:60,fontSize:13}}>
          <div style={{width:32,height:32,border:"3px solid #1a2035",borderTopColor:"#ff9800",
      ...
      {sorted.map(d=>{
        ...
        return(
          <div key={d.sym} style={{display:"grid",
            gridTemplateColumns:"100px 80px 70px 70px 70px 80px 90px 80px 1fr",
            gap:"0 8px",padding:"8px 8px",alignItems:"center",marginBottom:3,
            background:isCoil?"#1a1535":d.squeezeRisk?"#1a0d00":"#0c111e",
            borderRadius:6,border:`1px solid ${isCoil?"#7986cb33":d.squeezeRisk?"#ff980033":"#111827"}`,
            boxShadow:isCoil?"0 0 8px #7986cb22":"none"}}>
            ...
            <span style={{fontSize:11,color:"#90a4ae",fontVariantNumeric:"tabular-nums"}}>
              {d.price>=1000?d.price.toFixed(1):d.price>=1?d.price.toFixed(3):d.price.toFixed(6)}
            </span>

            <span style={{fontSize:11,fontWeight:700,color:oiColor}}>
              {fmtChg(d.oiChg1h)}
            </span>

            <span style={{fontSize:11,fontWeight:700,color:o4Color}}>
              {fmtChg(d.oiChg4h)}
            </span>

            <span style={{fontSize:11,color:d.change24>0?"#4caf50":d.change24<0?"#ef5350":"#90a4ae",fontWeight:700}}>
              {fmtChg(d.change24)}
            </span>

            <span style={{fontSize:11,color:frColor,fontWeight:700}}>
              {d.frNow!=null?`${d.frNow.toFixed(4)}%`:"—"}
            </span>

            <span style={{fontSize:10,color:"#546e7a"}}>
              {fmtOI(d.oiNow * d.price)}
            </span>

            <OISparkline data={d.oiHist} color={d.oiChg4h>0?"#4caf50":"#ef5350"}/>

            <div>
              <span style={{fontSize:10,color:d.divColor,fontWeight:d.divergence!=="neutral"?700:400}}>
                {d.divNote||"—"}
              </span>
            </div>
          </div>
      ...
      {/* 說明 */}
      <div style={{marginTop:20,background:"#0c111e",borderRadius:8,padding:"14px 16px",border:"1px solid #1a2035"}}>
        <div style={{fontSize:9,color:"#ff9800",letterSpacing:2,marginBottom:10}}>📖 OI 背離訊號說明</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[
            ["🔥 蓄勢待爆發","價格橫盤但OI急速變動 → 方向即將決定，最值得關注","#7986cb"],
            ["🚀 多頭積累","價漲+OI升 → 新多單持續進場，健康上漲趨勢","#4caf50"],
            ["💥 空頭積累","價跌+OI升 → 新空單進場，繼續下跌風險高","#ef5350"],
            ["🔄 空頭回補","價漲+OI降 → 空頭認賠平倉推升，持續性待觀察","#ff9800"],
            ["⚡ 軋倉風險","費率極端(>0.08%)+OI高 → 多空擇一方向被大幅軋倉","#ff9800"],
            ["📉 多頭止損","價跌+OI降 → 多頭認賠出場，可能接近底部","#ffb74d"],
          ].map(([t,d,c])=>(
            <div key={t} style={{background:"#060c1a",borderRadius:6,padding:"8px 10px"}}>
              <div style={{fontSize:10,color:c,fontWeight:700,marginBottom:3}}>{t}</div>
              <div style={{fontSize:10,color:"#546e7a",lineHeight:1.6}}>{d}</div>
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  const [page,        setPage]        = useState("scanner"); // "scanner" | "oi" | "journal"
  const [quickAdd,    setQuickAdd]    = useState(null); // data to auto-fill journal form
  ...
  useEffect(()=>{
    if(autoRefresh){
      // 每2分鐘自動掃描（開著網頁即可，有新訊號自動彈出）
      timerRef.current=setInterval(()=>runScan(),2*60*1000);
  ...
  const C={
    ...
    sL:{fontSize:10,letterSpacing:3,color:"#1e3a5f",textTransform:"uppercase",marginBottom:7},
    ...
    fB:(a,col)=>({padding:"4px 9px",borderRadius:4,fontSize:10,fontFamily:"inherit",cursor:"pointer",
      border:`1px solid ${a?(col||"#3949ab"):"#1a2035"}`, fontSize:11,
  ...
  return(
    <div style={{display:"flex",height:"100vh",overflow:"hidden",
      ...
      <style>{`
        ...
        @keyframes slideIn{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}} @keyframes slideInRight{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}
      ...
      {/* 右側固定通知欄 */}
      {alerts.length>0&&!newsSymbol&&(
        <AlertSidebar alerts={alerts} onDismiss={i=>{
          if(i==="all") setAlerts([]);
      ...
      <div style={C.sidebar}>
        <div>
          ...
          <div style={{fontSize:8,color:"#263238",marginTop:2}}>10項評分·SMC·K線·量價·OI</div>
          {/* 頁面切換 */}
          <div style={{display:"flex",flexDirection:"column",gap:4,marginTop:10}}>
            {[["scanner","📊 掃描器"],["oi","📈 OI儀表板"],["journal","📋 交易日誌"]].map(([v,l])=>(
              <button key={v} onClick={()=>setPage(v)}
                style={{width:"100%",padding:"7px 10px",borderRadius:5,fontSize:11,fontFamily:"inherit",cursor:"pointer",
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:11,color:autoRefresh?"#4caf50":"#37474f"}}>
            {autoRefresh?"🔴 即時監控中（每2分鐘）":"每2分鐘自動掃描"}
          </span>
      ...
      {page==="oi" ? <OIDashboard/> : page==="journal" ? <TradingJournal quickAdd={quickAdd} onQuickAddDone={()=>setQuickAdd(null)}/> : (
      <div style={C.main}>
        {/* 篩選列 */}
        ...
        {/* 表頭 */}
        {filtered.length>0&&(
          <div style={{display:"grid",gridTemplateColumns:"26px 42px 1fr 88px 88px 88px 88px 60px 80px 60px",
        ...
        {filtered.map((r,idx)=>(
          <div key={r.symbol} style={C.card(r.score,expanded===r.symbol,r.surge,r.crash)}>
            {/* 主列 */}
            <div style={{display:"grid",gridTemplateColumns:"26px 42px 1fr 88px 88px 88px 88px 60px 80px 60px",
              gap:"0 6px",padding:"9px 6px",alignItems:"center"}}
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                  <span style={{fontSize:14,fontWeight:700,color:"#e8eaf6"}}>{r.symbol.replace("USDT","")}</span>
                ...
                <button
                  onClick={e=>{
                    ...
                    setQuickAdd({
                      ...
                      note:       `${r.grade} · ${r.smc?.smcVerdict||""} · ${r.vpt?.verdict||""}`.trim(),
                    });
                    setPage("journal");
                  }}
                  title="快速記錄到日誌"
                  style={{padding:"3px 7px",borderRadius:4,border:"1px solid #1a2035",
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
                  ...
                  <div>
                    ...
                    <div style={{marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#37474f",marginBottom:4}}>
                        <span>爆發評分</span>
                        <span style={{color:r.gradeColor,fontWeight:800,fontSize:15}}>{r.score}/100</span>
                      </div>
                    ...
                    {(()=>{
                      ...
                      if(!dec) return null;
                      return(
                        <div style={{background:dec.decisionBg,borderRadius:10,padding:"12px 14px",
                          border:`2px solid ${dec.decisionColor}55`,marginBottom:10}}>
                          <div style={{fontSize:22,fontWeight:900,color:dec.decisionColor,marginBottom:4}}>
                            {dec.decision}
                          </div>
                          <div style={{fontSize:12,color:"#546e7a",marginBottom:10}}>{dec.urgency}</div>
                          {dec.checks.map((ck,j)=>(
                            <div key={j} style={{display:"flex",gap:7,marginBottom:5,alignItems:"flex-start"}}>
                              <span style={{fontSize:13,minWidth:18}}>
                                {ck.ok?"✅":ck.must?"❌":"⚠️"}
                              </span>
                              <div>
                                <span style={{fontSize:11,fontWeight:700,
                                  color:ck.ok?"#4dd0e1":ck.must?"#ef5350":"#ff9800"}}>
                                  {ck.label}{ck.must?" ★":""}
                                </span>
                                <div style={{fontSize:10,color:"#546e7a",lineHeight:1.5}}>{ck.note}</div>
                              </div>
                            </div>
                          ))}
                          <div style={{marginTop:8,fontSize:10,color:"#37474f",
                            borderTop:"1px solid #1a2035",paddingTop:6}}>
                            ★ 必要條件缺一不可 · 加分條件越多越好
                          </div>
                        </div>
                      );
                    ...
                    <button onClick={e=>{
                      ...
                      setQuickAdd({
                        ...
                        smcVerdict: r.smc?.smcVerdict    || "",
                        vptVerdict: r.vpt?.verdict       || "",
                        note:`評${r.score}分 · ${r.smc?.smcVerdict||""} · ${r.vpt?.verdict||""}`,
                      });
                      setPage("journal");
                    }}
                      style={{width:"100%",padding:"9px",borderRadius:6,
                        ...
                        fontSize:12,fontWeight:700,fontFamily:"inherit",marginBottom:8,textAlign:"left"}}>
                      📋 一鍵記錄到交易日誌（自動帶入所有數據）
                    </button>
