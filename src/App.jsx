      ...
      {trades.length===0?(
        ...
        <>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10,
            ...
            <input type="number" defaultValue="50" id="capitalInput"
            ...
            <input type="number" defaultValue="10" id="leverageInput"
            ...
            <span style={{fontSize:10,color:"#37474f"}}>倍 · 倉位價值 <span style={{color:"#4dd0e1",fontWeight:700}}>
              {(50*10).toFixed(0)}U
            </span></span>
            <span style={{fontSize:9,color:"#263238",marginLeft:8}}>（損益 = 倉位價值 × 漲跌%）</span>
          </div>
          ...
          {trades.map(t=>{
            ...
            // 計算實際損益U（讀取用戶輸入的資金和槓桿）
            const calcPnlU = (pctStr) => {
              const capital = parseFloat(document.getElementById("capitalInput")?.value||"50");
              const leverage = parseFloat(document.getElementById("leverageInput")?.value||"10");
              const pct = parseFloat(pctStr||"0");
              return (capital * leverage * pct / 100).toFixed(2);
            };
              ...
              <div key={t.id} style={{display:"grid",
                ...
                <div>
                  {res&&(()=>{
                    ...
                    const pnlU = calcPnlU(res.currentPnl);
                    const maxPU = calcPnlU(res.maxProfit);
                    const isPos = pnl > 0;
                    const col = isPos?"#4caf50":pnl<0?"#ef5350":"#90a4ae";
                    ...
                    else if(res.result==="SL") finalPct = t.slPct;
                    const finalU = calcPnlU(finalPct);
                    const isClosed = ["TP1","TP2","TP3","SL"].includes(res.result);
                        ...
                        {isClosed?(
                          <>
                            <div style={{fontSize:13,fontWeight:900,color:col}}>
                              {parseFloat(finalPct)>0?"+":""}{finalPct}%
                            </div>
                            <div style={{fontSize:13,fontWeight:800,
                              color:parseFloat(finalU)>0?"#4caf50":"#ef5350",
                              background:parseFloat(finalU)>0?"#4caf5018":"#ef535018",
                              padding:"2px 5px",borderRadius:4,marginTop:2,display:"inline-block"}}>
                          ...
                          <>
                            ...
                            <div style={{fontSize:9,color:"#37474f"}}>最高+{maxPU}U</div>
                          </>
                        )}
                      </div>
                    );
