...
function TradingJournal({ quickAdd, onQuickAddDone }) {
  ...
  // Use formRef to avoid stale closure - always read latest form values
  const formRef = useRef(form);
  useEffect(()=>{ formRef.current = form; }, [form]);

  const handleAddTrade = () => {
    const f = formRef.current; // always fresh
    ...
    // Load fresh from storage to avoid overwriting existing records
    ...
    setForm(f => ({
      ...f,
      entryPrice:"", stopLoss:"", tp1:"", tp2:"", tp3:"", note:"", score:""
    }));
  };
  ...
  const checkResult = async (trade) => {
    ...
    const updated = existing.map(t=>
      t.id===trade.id ? {...t, result, checkedAt:new Date().toISOString()} : t
    );
    saveJournal(updated);
    setTrades(updated);
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>{
            ...
            style={{padding:"8px 14px",borderRadius:6,border:"1px solid #1a2035",
              background:"#0c111e",color:"#7986cb",cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>
            🔄 同步
          </button>
