...
// 暴漲 / 暴跌 預警偵測
// ══════════════════════════════════════════════════════════════════
function detectSurge(candles, last, ma30, ma45, fundingRate) {
  const closes  = candles.map(c=>c.c);
  const vols    = candles.map(c=>c.v);
  ...
  const obvPrev = obv[obv.length-6]; // 5根前
  ...
  const priceSlope = price5ago ? (last-price5ago)/price5ago : 0;

  // 近30日最高（突破判斷）
  const hi30 = Math.max(...highs.slice(-30));
  const breakout = last >= hi30 * 0.998; // 接近或突破30日高點

  // BB 收窄後放量（Squeeze 爆發）
  const bw = bollingerWidth(closes, 20);
  const bwPrev = bollingerWidth(closes.slice(0,-3), 20);
  const squeezeBreak = bw!=null && bwPrev!=null && bwPrev<0.05 && bw>bwPrev*1.3 && vr>2;

  // RSI 快速拉升
  const rsiNow  = rsiCalc(closes, 14);
  const rsiPrev = rsiCalc(closes.slice(0,-3), 14);
  const rsiSurge = rsiNow!=null && rsiPrev!=null && rsiNow-rsiPrev>15 && rsiNow>50;

  const surgeReasons = [];
  let surgeScore = 0;

  if(vr>3)          { surgeScore+=3; surgeReasons.push(`量比${vr.toFixed(1)}x 異常爆量`); }
  else if(vr>2)     { surgeScore+=1; surgeReasons.push(`量比${vr.toFixed(1)}x 放量`); }
  if(obvSurge>0.3)  { surgeScore+=2; surgeReasons.push(`OBV急速上升${(obvSurge*100).toFixed(0)}%`); }
  if(breakout)      { surgeScore+=3; surgeReasons.push(`突破近30日高點 ${hi30.toFixed(4)}`); }
  if(squeezeBreak)  { surgeScore+=3; surgeReasons.push(`布林帶Squeeze後爆量突破`); }
  if(rsiSurge)      { surgeScore+=2; surgeReasons.push(`RSI急速拉升+${(rsiNow-rsiPrev).toFixed(0)}pt`); }
  if(priceSlope>0.05){ surgeScore+=2; surgeReasons.push(`近5根漲幅${(priceSlope*100).toFixed(1)}%`); }

  const isSurge = surgeScore >= 5;
  const surgeStrength = surgeScore>=8?"🚀🚀 極強爆漲":surgeScore>=5?"🚀 暴漲預警":"";

  return { isSurge, surgeScore, surgeStrength, surgeReasons };
}

function detectCrash(candles, last, ma30, ma45, fundingRate) {
  const closes = candles.map(c=>c.c);
  const vols   = candles.map(c=>c.v);
  const highs  = candles.map(c=>c.h);
  ...
  // OBV 頂背離：價格創新高但 OBV 未創新高
  const hi10   = Math.max(...highs.slice(-10));
  const hi20   = Math.max(...highs.slice(-20,-10));
  const obvHi10= Math.max(...obv.slice(-10));
  const obvHi20= Math.max(...obv.slice(-20,-10));
  const obvDiverg = hi10>hi20 && obvHi10<obvHi20; // 價格新高，OBV不新高
  ...
  const upperShadow = lastC.h-Math.max(lastC.o,lastC.c);
  const body         = Math.abs(lastC.c-lastC.o);
  const longUpperShadow = upperShadow > body*2 && upperShadow/lastC.h > 0.02;

  // RSI 超買 + 量縮
  const rsiNow = rsiCalc(closes, 14);
  const shrink = vol20avg ? (vol20avg-vols[vols.length-1])/vol20avg : 0;
  const rsiOverbought = rsiNow!=null && rsiNow>75 && shrink>0.2;

  // 跌破 MA30 後反彈失敗
  const prevClose  = closes[closes.length-2];
  const maBelowBreak = prevClose>ma30 && last<ma30;
  ...
  // 資金費率過高（多頭過熱）
  const frOverheat = fundingRate!=null && fundingRate>0.08;

  const crashReasons = [];
  let crashScore = 0;

  if(obvDiverg)       { crashScore+=3; crashReasons.push(`OBV頂背離，資金偷跑`); }
  if(longUpperShadow) { crashScore+=3; crashReasons.push(`高位長上影線，拉高出貨`); }
  if(rsiOverbought)   { crashScore+=3; crashReasons.push(`RSI超買${rsiNow?.toFixed(0)}+量縮，動能衰竭`); }
  if(maBelowBreak)    { crashScore+=2; crashReasons.push(`剛跌破MA30，趨勢轉弱`); }
  if(vr>2&&priceSlope<0){ crashScore+=3; crashReasons.push(`放量${vr.toFixed(1)}x下跌，空方強勢`); }
  if(frOverheat)      { crashScore+=2; crashReasons.push(`資金費率${fundingRate?.toFixed(4)}%過熱，易被軋`); }
  if(priceSlope<-0.05){ crashScore+=2; crashReasons.push(`近5根跌幅${(Math.abs(priceSlope)*100).toFixed(1)}%`); }

  const isCrash = crashScore >= 5;
  const crashStrength = crashScore>=8?"💥💥 極危":crashScore>=5?"💥 暴跌預警":"";

  return { isCrash, crashScore, crashStrength, crashReasons };
}
...
async function analyseSymbol(symbol, interval) {
  ...
  try {
    if(frVal!=null){
      extras.push({label:"資金費率",value:`${frVal.toFixed(4)}%`,
        note:frVal>0.01?"正費率（偏熱）":frVal<-0.01?"負費率（潛在軋空）":"中性",
  ...
  return { symbol,score,grade,gradeColor,price:last.toFixed(4),
    ...
    aboveAll,maFan,isEntry,signals,extras,vpt,trade,surge,crash,news:[] };
}
...
function EntryAlert({ alerts, onDismiss }) {
  ...
  return (
    <div style={{ position:"fixed", top:20, right:20, zIndex:9999, display:"flex",
      ...
      {alerts.map((a,i)=>{
        const isSurge = a.alertType==="surge";
        const isCrash = a.alertType==="crash";
        const borderCol = isCrash?"#ef5350":isSurge?"#ff9800":"#4caf50";
        const bgGrad = isCrash?"linear-gradient(135deg,#1a0505,#1a0a00)":
                       isSurge?"linear-gradient(135deg,#1a1000,#1a1500)":
                               "linear-gradient(135deg,#0d2137,#0d2820)";
        const title = isCrash?"💥 暴跌預警":isSurge?"🚀 暴漲預警":"🚀 進場訊號觸發";
        const reasons = isCrash?a.crash?.crashReasons:isSurge?a.surge?.surgeReasons:null;
        const strength = isCrash?a.crash?.crashStrength:isSurge?a.surge?.surgeStrength:"";
        return (
          <div key={a.symbol+i} style={{
            background:bgGrad, border:`1px solid ${borderCol}`,
            borderRadius:12, padding:"14px 16px",
            boxShadow:`0 8px 32px #00000088,0 0 20px ${borderCol}44`,
            animation:"slideIn .3s ease" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              <div style={{flex:1}}>
                <div style={{ fontSize:11, color:borderCol, letterSpacing:2, marginBottom:4 }}>{title}</div>
                <div style={{ fontSize:18, fontWeight:800, color:"#e8eaf6", marginBottom:6 }}>
                  {a.symbol.replace("USDT","")}
                  {strength&&<span style={{ fontSize:12, marginLeft:8, color:borderCol }}>{strength}</span>}
                </div>
                {reasons&&reasons.slice(0,3).map((r,j)=>(
                  <div key={j} style={{ fontSize:11, color:"#546e7a", marginBottom:3 }}>
                    {isCrash?"▼":"▲"} {r}
                  </div>
                ))}
                {!isSurge&&!isCrash&&a.trade&&(
                  <div style={{ background:"#060c1a", borderRadius:6, padding:"8px 10px",
                    border:`1px solid ${borderCol}33`, fontSize:10, marginTop:8 }}>
                    <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                      <span>📍<span style={{ color:"#e8eaf6", fontWeight:700 }}>{a.trade.entryIdeal}</span></span>
                      <span>🛑<span style={{ color:"#ef5350", fontWeight:700 }}>{a.trade.stopLoss}</span>({a.trade.slPct}%)</span>
                      <span>🎯<span style={{ color:"#4caf50", fontWeight:700 }}>{a.trade.tp1}</span>(+{a.trade.tp1Pct}%)</span>
                    </div>
                    <div style={{ color:"#455a64", marginTop:4 }}>風報比 1:{a.trade.rr}</div>
                  </div>
                )}
              </div>
    ...
    try {
      ...
      for(let i=0;i<symList.length;i+=BATCH){
        ...
        res.forEach(r=>{
          ...
          out.push(r);
          // 進場訊號通知（每個幣種只通知一次）
          if(r.isEntry && !seenAlerts.current.has(r.symbol+iv+"entry")){
            seenAlerts.current.add(r.symbol+iv+"entry");
          ...
          if(r.surge?.isSurge && !seenAlerts.current.has(r.symbol+iv+"surge")){
            seenAlerts.current.add(r.symbol+iv+"surge");
            newAlerts.push({...r, alertType:"surge"});
            if(Notification.permission==="granted"){
              new Notification(`🚀 暴漲預警：${r.symbol.replace("USDT","")}`,{
                body:r.surge.surgeReasons.slice(0,2).join(" · "),
                icon:"/icon-192.png"
              });
            }
          }
          if(r.crash?.isCrash && !seenAlerts.current.has(r.symbol+iv+"crash")){
            seenAlerts.current.add(r.symbol+iv+"crash");
            newAlerts.push({...r, alertType:"crash"});
            if(Notification.permission==="granted"){
              new Notification(`💥 暴跌預警：${r.symbol.replace("USDT","")}`,{
                body:r.crash.crashReasons.slice(0,2).join(" · "),
                icon:"/icon-192.png"
              });
            }
          }
        ...
        {results.length>0&&!loading&&(
          <div style={{background:"#060810",borderRadius:7,padding:"10px 12px",border:"1px solid #0f1629",fontSize:10}}>
            ...
            {[["掃描幣種",progress.total||results.length],["有效結果",results.length],
              ...
              ["🚀進場訊號",entryCount],
              ["🚀暴漲預警",results.filter(r=>r.surge?.isSurge).length],
              ["💥暴跌預警",results.filter(r=>r.crash?.isCrash).length],
              ["多頭排列",results.filter(r=>r.maFan).length]
            ].map(([k,v])=>(
              <div key={k} style={{display:"flex",justifyContent:"space-between",color:"#455a64",marginBottom:4}}>
                <span>{k}</span>
                <span style={{color:(k==="🚀進場訊號"||k==="🚀暴漲預警")&&v>0?"#4caf50":k==="💥暴跌預警"&&v>0?"#ef5350":"#7986cb",fontWeight:700}}>{v}</span>
              </div>
            ))}
          ...
          <div style={{display:"flex",gap:4}}>
            {[["all","全部"],
              ...
              <button key={v} onClick={()=>setFilter(v)}
                style={{...C.fBtn(filter===v),
                  color:filter===v?(v==="crash"?"#ef9a9a":v==="surge"?"#ffb74d":undefined):
                    (v==="crash"?"#ef535066":v==="surge"?"#ff980066":undefined)}}>
                {l}
              </button>
            ))}
          </div>
            ...
            <div style={{display:"grid",
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                  ...
                  {r.surge?.isSurge&&<span style={{fontSize:8,color:"#ff9800",background:"#ff980022",padding:"1px 5px",borderRadius:3,fontWeight:700}}>{r.surge.surgeStrength||"🚀暴漲"}</span>}
                  {r.crash?.isCrash&&<span style={{fontSize:8,color:"#ef5350",background:"#ef535022",padding:"1px 5px",borderRadius:3,fontWeight:700}}>{r.crash.crashStrength||"💥暴跌"}</span>}
            ...
            {expanded===r.symbol&&(
              <div style={{borderTop:"1px solid #0f1629",padding:"14px 16px",
                ...
                <div>
                  ...
                  {/* 暴漲 / 暴跌預警詳情 */}
                  {(r.surge?.isSurge || r.crash?.isCrash) && (
                    <div style={{marginBottom:10}}>
                      {r.surge?.isSurge && (
                        <div style={{background:"#1a1000",borderRadius:7,padding:"10px 12px",
                          border:"1px solid #ff980044",marginBottom:6}}>
                          <div style={{fontSize:9,color:"#ff9800",letterSpacing:2,marginBottom:6}}>
                            🚀 {r.surge.surgeStrength} · 爆量得分 {r.surge.surgeScore}
                          </div>
                          {r.surge.surgeReasons.map((rs,j)=>(
                            <div key={j} style={{fontSize:10,color:"#546e7a",marginBottom:3}}>▲ {rs}</div>
                          ))}
                        </div>
                      )}
                      {r.crash?.isCrash && (
                        <div style={{background:"#1a0505",borderRadius:7,padding:"10px 12px",
                          border:"1px solid #ef535044"}}>
                          <div style={{fontSize:9,color:"#ef5350",letterSpacing:2,marginBottom:6}}>
                            💥 {r.crash.crashStrength} · 崩跌得分 {r.crash.crashScore}
                          </div>
