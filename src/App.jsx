...
// Safe notification - iOS Safari doesn't support Notification API
...
function KlineChart({ candles, ma30arr, ma45arr, ma60arr, bb, trade, smc, height=280 }) {
  ...
  useEffect(()=>{
    ...
    } catch(e){ console.warn("KlineChart:",e); }
...
function detectCrash(candles,last,ma30,ma45,fr) {
  ...
  const body=Math.abs(lastC.c-lastC.o)||0.0001;
  const longUpper=uShadow>body*2&&uShadow/lastC.h>0.015;

  // 下影線極小（賣壓沉重）
  const lShadow=Math.min(lastC.o,lastC.c)-lastC.l;
  const heavySell=lastC.c<lastC.o&&lShadow<body*0.3;

  const rsiNow=rsiCalc(closes);
  const rsiPrev=rsiCalc(closes.slice(0,-3));
  const shrink=vol20avg?(vol20avg-vols[vols.length-1])/vol20avg:0;
  const rsiOB=rsiNow&&rsiNow>72&&shrink>0.15;
  // RSI 由高位快速下跌（動能反轉）
  const rsiTopRev=rsiNow&&rsiPrev&&rsiPrev>68&&rsiNow<rsiPrev-8;

  // 跌破均線
  const maBelowMA30=closes[closes.length-2]>ma30&&last<ma30;
  const maBelowMA45=closes[closes.length-2]>ma45&&last<ma45;
  // 均線死叉（MA30跌破MA45）
  const maDead=ma30<ma45;

  const slope5=closes.length>=6?(last-closes[closes.length-6])/closes[closes.length-6]:0;
  const slope10=closes.length>=11?(last-closes[closes.length-11])/closes[closes.length-11]:0;
  const frOverheat=fr!=null&&fr>0.06;
  const frNeg=fr!=null&&fr<-0.05; // 負費率但OI高 = 空頭過多易軋空

  // 布林帶下穿（收窄後向下爆發）
  const bb=bollingerBands(closes,20);
  const bbBreakDown=bb&&last<bb.lower;

  // OBV 連續下跌
  const obvNow=obv[obv.length-1], obvPrev5=obv[obv.length-6]||obvNow;
  const obvFalling=obvNow<obvPrev5*0.97;

  // 量價背離（價漲量縮後轉跌）
  const recentUp=closes.slice(-5).every((c,i,a)=>i===0||c>=a[i-1]);
  const volShrinkOnUp=recentUp&&shrink>0.2;

  const reasons=[]; let s=0;
  if(obvDiverg)    {s+=3;reasons.push("⚠️ OBV頂背離，主力資金偷跑中");}
  if(longUpper)    {s+=3;reasons.push("⚠️ 高位長上影線，明顯拉高出貨");}
  if(heavySell)    {s+=2;reasons.push("⚠️ 大陰線且下影極短，賣壓沉重");}
  if(rsiOB)        {s+=3;reasons.push(`⚠️ RSI超買${rsiNow?.toFixed(0)}且量縮，動能衰竭`);}
  if(rsiTopRev)    {s+=2;reasons.push(`⚠️ RSI從高位急跌${(rsiPrev-rsiNow).toFixed(0)}pt，趨勢反轉`);}
  if(maBelowMA30)  {s+=3;reasons.push("🔴 剛跌破MA30，趨勢轉空");}
  if(maBelowMA45)  {s+=2;reasons.push("🔴 跌破MA45，空頭加速");}
  if(maDead)       {s+=2;reasons.push("🔴 MA30下穿MA45，均線死叉");}
  if(vr>2&&slope5<0){s+=3;reasons.push(`🔴 放量${vr.toFixed(1)}x下跌，空方強勢`);}
  if(frOverheat)   {s+=2;reasons.push(`⚠️ 資金費率${fr?.toFixed(4)}%過熱，多頭擁擠易被軋`);}
  if(bbBreakDown)  {s+=2;reasons.push("🔴 收盤跌破布林下軌，超賣加速");}
  if(obvFalling)   {s+=2;reasons.push("⚠️ OBV持續下降，資金持續流出");}
  if(volShrinkOnUp){s+=2;reasons.push("⚠️ 近期上漲量縮，假突破風險高");}
  if(slope5<-0.05) {s+=2;reasons.push(`🔴 近5根跌幅${(Math.abs(slope5)*100).toFixed(1)}%`);}
  if(slope10<-0.08){s+=1;reasons.push(`🔴 近10根跌幅${(Math.abs(slope10)*100).toFixed(1)}%，中期轉空`);}

  // 準備下跌預警（分數較低但有明確訊號）
  const isBearSetup = s>=3 && s<5;
  return {
    isCrash:s>=5, isBearSetup,
    crashScore:s,
    crashStrength:s>=10?"💥💥 極度危險":s>=7?"💥 強烈暴跌訊號":s>=5?"💥 暴跌預警":s>=3?"⚠️ 準備下跌":"",
    crashReasons:reasons
  };
}
  ...
  render(){
    if(this.state.hasError) return(
      <div style={{height:"100vh",display:"flex",flexDirection:"column",
        ...
        color:"#90a4ae",fontFamily:"monospace",padding:20,textAlign:"center"}}>
        <div style={{fontSize:40,marginBottom:16}}>⚠️</div>
        <div style={{fontSize:16,fontWeight:700,color:"#ef5350",marginBottom:8}}>載入發生錯誤</div>
        <div style={{fontSize:11,color:"#546e7a",marginBottom:20}}>{this.state.error}</div>
        <button onClick={()=>window.location.reload()}
          style={{padding:"10px 24px",borderRadius:8,border:"none",
            background:"#283593",color:"#fff",cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>
          重新載入
        </button>
      </div>
    );
...
function TradingJournal({quickAdd,onQuickAddDone}){
  ...
  const [form,setForm]=useState({
    ...
    entryTime:new Date(Date.now() + 8*60*60*1000).toISOString().slice(0,16),note:"",score:"",smcVerdict:"",vptVerdict:""
  });
  ...
  useEffect(()=>{
    if(!quickAdd) return;
    // Merge quickAdd with all required form fields (ensure nothing is undefined)
    const newForm={
      symbol:      quickAdd.symbol      || "BTCUSDT",
      direction:   quickAdd.direction   || "long",
      entryPrice:  String(quickAdd.entryPrice || ""),
      stopLoss:    String(quickAdd.stopLoss   || ""),
      tp1:         String(quickAdd.tp1        || ""),
      tp2:         String(quickAdd.tp2        || ""),
      tp3:         String(quickAdd.tp3        || ""),
      score:       String(quickAdd.score      || ""),
      smcVerdict:  quickAdd.smcVerdict  || "",
      vptVerdict:  quickAdd.vptVerdict  || "",
      note:        quickAdd.note        || "",
      entryTime:   new Date(Date.now()+8*60*60*1000).toISOString().slice(0,16),
    };
    setForm(newForm);
  ...
  const closed=trades.filter(t=>["TP1","TP2","TP3","SL","手動平倉"].includes(t.result?.result));
  const wins=closed.filter(t=>t.result?.result?.startsWith("TP")||t.result?.result==="手動平倉"&&parseFloat(t.result?.currentPnl||0)>0);
  const losses=closed.filter(t=>t.result?.result==="SL"||(t.result?.result==="手動平倉"&&parseFloat(t.result?.currentPnl||0)<=0));
  ...
  const handleAdd=()=>{
    ...
    const newTrade={...f,id:Date.now(),slPct,tp1Pct,tp2Pct,tp3Pct,result:null,addedAt:new Date().toLocaleString("zh-TW",{timeZone:"Asia/Taipei"})};
  ...
  // 手動平倉
  const doClose=(trade)=>{
    const closePrice=parseFloat(closeInput[trade.id]||"");
    if(isNaN(closePrice)||closePrice<=0){alert("請輸入平倉價格");return;}
    const ep=parseFloat(trade.entryPrice);
    const dir=trade.direction==="long"?1:-1;
    const pct=((closePrice-ep)/ep*100*dir).toFixed(2);
    const pnlU=(capital*leverage*parseFloat(pct)/100).toFixed(2);
    const result={
      result:"手動平倉",
      resultTime:new Date().toLocaleString("zh-TW",{timeZone:"Asia/Taipei"}),
      currentPnl:pct,
      maxProfit:pct,
      maxLoss:pct,
      lastPrice:closePrice.toFixed(4),
    ...
    const updated=existing.map(t=>t.id===trade.id?{...t,result,checkedAt:new Date().toISOString()}:t);
    saveJournal(updated);setTrades(updated);
    setCloseInput(c=>({...c,[trade.id]:""}));
  };

  // 同步（修復：強制重新讀取 localStorage）
  const sync=()=>{
    const fresh=loadJournal();
    setTrades([...fresh]);
  };

  const rCol=r=>r==="TP3"?"#2e7d32":r==="TP2"?"#4caf50":r==="TP1"?"#8bc34a":r==="SL"?"#ef5350":r==="手動平倉"?"#7986cb":r==="持倉中"?"#ff9800":"#37474f";
  const rLabel=r=>r==="TP3"?"🎯🎯 TP3":r==="TP2"?"🎯 TP2":r==="TP1"?"✅ TP1":r==="SL"?"🛑 止損":r==="手動平倉"?"✋ 手動平倉":r==="持倉中"?"⏳ 持倉中":"—";
      ...
      {trades.length===0?(
        ...
        <>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10,
            background:"#0c111e",borderRadius:7,padding:"8px 14px",border:"1px solid #1a2035"}}>
            <span style={{fontSize:10,color:"#37474f"}}>每單資金</span>
            <input type="number" value={capital} onChange={e=>setCapital(parseFloat(e.target.value)||50)}
              style={{width:70,background:"#060810",border:"1px solid #1a2035",borderRadius:4,
                color:"#ff9800",fontSize:12,padding:"4px 6px",fontFamily:"inherit",outline:"none",fontWeight:700}}/>
            <span style={{fontSize:10,color:"#37474f"}}>U · 槓桿</span>
            <input type="number" value={leverage} onChange={e=>setLeverage(parseFloat(e.target.value)||10)}
              style={{width:50,background:"#060810",border:"1px solid #1a2035",borderRadius:4,
                color:"#7986cb",fontSize:12,padding:"4px 6px",fontFamily:"inherit",outline:"none",fontWeight:700}}/>
            <span style={{fontSize:10,color:"#37474f"}}>倍 · 倉位 <span style={{color:"#4dd0e1",fontWeight:700}}>{pos.toFixed(0)}U</span></span>
            <span style={{fontSize:9,color:"#263238",marginLeft:8}}>損益 = 倉位 × 漲跌%</span>
          </div>

          <div style={{display:"grid",gridTemplateColumns:"95px 50px 80px 80px 65px 65px 65px 1fr 100px 130px 65px",
            ...
            <span>回測結果</span><span>損益% / U</span><span>操作</span>
          </div>
          {trades.map(t=>{
            const res=t.result;
            ...
            // 計算實際損益U（使用 React state）
            const calcPnlU = (pctStr) => {
              const pct = parseFloat(pctStr||"0");
              return (pos * pct / 100).toFixed(2);
            };

            return(
              <div key={t.id} style={{display:"grid",
                gridTemplateColumns:"95px 50px 80px 80px 65px 65px 65px 1fr 100px 130px 65px",
                ...
                <div>
                  ...
                  <div style={{fontSize:9,color:"#37474f"}}>{(()=>{
                    ...
                    // entryTime stored as local UTC+8 input
                    return dt.toLocaleString("zh-TW",{timeZone:"Asia/Taipei",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});
                  })()}</div>
                </div>
                ...
                <div>
                  {res?(<>
                    <div style={{fontSize:12,fontWeight:700,color:rCol(res.result)}}>{rLabel(res.result)}</div>
                    {res.resultTime&&<div style={{fontSize:9,color:"#37474f"}}>{res.resultTime} (UTC+8)</div>}
                ...
                <div>
                  {res&&(()=>{
                    const pnl=parseFloat(res.currentPnl||"0");
                    const pnlU=(pos*pnl/100).toFixed(2);
                    const col=pnl>0?"#4caf50":pnl<0?"#ef5350":"#90a4ae";
                    const isClosed=["TP1","TP2","TP3","SL","手動平倉"].includes(res.result);

                    let finalPct=res.currentPnl;
                    if(res.result==="TP3") finalPct=t.tp3Pct;
                    else if(res.result==="TP2") finalPct=t.tp2Pct;
                    else if(res.result==="TP1") finalPct=t.tp1Pct;
                    else if(res.result==="SL") finalPct=("-"+Math.abs(parseFloat(t.slPct)));
                    // 手動平倉用 currentPnl
                    const finalU=(pos*parseFloat(finalPct||"0")/100).toFixed(2);
                    const finalCol=parseFloat(finalU)>=0?"#4caf50":"#ef5350";

                    return(
                      <div>
                        {isClosed?(
                          <>
                            <div style={{fontSize:13,fontWeight:900,color:finalCol}}>
                              {parseFloat(finalPct)>0?"+":""}{parseFloat(finalPct).toFixed(2)}%
                            </div>
                            <div style={{fontSize:14,fontWeight:800,
                              color:finalCol,
                              background:parseFloat(finalU)>=0?"#4caf5022":"#ef535022",
                              padding:"3px 7px",borderRadius:5,marginTop:3,display:"inline-block",
                              border:`1px solid ${finalCol}44`}}>
                              {parseFloat(finalU)>=0?"+":""}{finalU} U
                            </div>
                          </>
                        ):(
                          <>
                            <div style={{fontSize:12,fontWeight:800,color:col}}>
                              {pnl>0?"+":""}{res.currentPnl}%
                            </div>
                            <div style={{fontSize:13,fontWeight:700,color:col}}>
                              {parseFloat(pnlU)>=0?"+":""}{pnlU} U
                            </div>
                            <div style={{fontSize:9,color:"#37474f",marginTop:1}}>
                              最高+{(pos*parseFloat(res.maxProfit||0)/100).toFixed(2)}U
                            </div>
                          </>
                        )}
                      </div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:4}}>
                  ...
                  {(!res||res.result==="持倉中")&&(
                    <div style={{display:"flex",flexDirection:"column",gap:3,
                      background:"#0d2137",borderRadius:5,padding:"5px 6px",
                      border:"1px solid #3949ab44",marginBottom:2}}>
                      <span style={{fontSize:8,color:"#37474f"}}>平倉價格</span>
                      <input type="number"
                        value={closeInput[t.id]||""}
                        onChange={e=>setCloseInput(c=>({...c,[t.id]:e.target.value}))}
                        placeholder={t.entryPrice}
                        style={{background:"#060810",border:"1px solid #1a2035",borderRadius:4,
                          color:"#9fa8da",fontSize:11,padding:"3px 5px",fontFamily:"inherit",outline:"none",width:"100%"}}/>
                      <button onClick={()=>doClose(t)}
                        style={{padding:"4px 6px",borderRadius:4,border:"none",
                          background:"linear-gradient(135deg,#7986cb,#5c6bc0)",
                          color:"#fff",cursor:"pointer",fontSize:10,fontFamily:"inherit",fontWeight:700}}>
                        ✋ 平倉
                      </button>
                    </div>
                  )}
          ...
          {closed.length>=3&&(()=>{
            ...
            const avgW=wins.length>0?(wins.reduce((a,t)=>{
              ...
              const p=r==="TP3"?parseFloat(t.tp3Pct||0):r==="TP2"?parseFloat(t.tp2Pct||0):r==="手動平倉"?parseFloat(t.result?.currentPnl||0):parseFloat(t.tp1Pct||0);
            ...
            const totalWinU=(wins.reduce((a,t)=>{
              const r=t.result?.result;
              const p=r==="TP3"?parseFloat(t.tp3Pct||0):r==="TP2"?parseFloat(t.tp2Pct||0):r==="手動平倉"?parseFloat(t.result?.currentPnl||0):parseFloat(t.tp1Pct||0);
              return a+pos*p/100;
            },0)).toFixed(2);
            const totalLossU=(losses.reduce((a,t)=>a+pos*Math.abs(parseFloat(t.slPct||0))/100,0)).toFixed(2);
            const netU=(parseFloat(totalWinU)-parseFloat(totalLossU)).toFixed(2);
            return(
              <div style={{marginTop:16,background:"#0c111e",borderRadius:10,padding:"14px 16px",border:"1px solid #1a2035"}}>
                ...
                {/* 總盈虧 */}
                <div style={{background:parseFloat(netU)>=0?"#0d2820":"#1a0505",borderRadius:8,padding:"12px 16px",
                  border:`1px solid ${parseFloat(netU)>=0?"#4caf5044":"#ef535044"}`,marginBottom:12,
                  display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:10,color:"#37474f",marginBottom:3}}>總淨利（{closed.length}筆已結算）</div>
                    <div style={{fontSize:28,fontWeight:900,color:parseFloat(netU)>=0?"#4caf50":"#ef5350"}}>
                      {parseFloat(netU)>=0?"+":""}{netU} U
                    </div>
                    <div style={{fontSize:11,color:"#37474f",marginTop:3}}>
                      資金設定：每單 {capital}U × {leverage}倍 = 倉位 {pos.toFixed(0)}U
                    </div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:12,color:"#4caf50",marginBottom:4}}>獲利 +{totalWinU} U</div>
                    <div style={{fontSize:12,color:"#ef5350"}}>虧損 -{totalLossU} U</div>
                  </div>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
                  ...
                  <div>
                    <div style={{fontSize:11,color:"#37474f",marginBottom:7}}>期望值（每筆平均）</div>
                    <div style={{fontSize:22,fontWeight:800,color:evPos?"#4caf50":"#ef5350"}}>{evPos?"+":""}{ev}%</div>
                    <div style={{fontSize:11,color:evPos?"#4caf50":"#ef5350",marginTop:2,fontWeight:700}}>
                      {evPos?"+":""}{(parseFloat(ev)*pos/100).toFixed(2)} U / 筆
                    </div>
                    <div style={{fontSize:9,color:"#37474f",marginTop:2}}>{evPos?"正期望值 ✅":"負期望值，需改善"}</div>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#37474f",marginBottom:7}}>均值</div>
                    <div style={{fontSize:11,color:"#4caf50"}}>平均獲利 +{avgW}% / +{(parseFloat(avgW)*pos/100).toFixed(2)}U</div>
                    <div style={{fontSize:11,color:"#ef5350",marginTop:4}}>平均虧損 -{avgL}% / -{(parseFloat(avgL)*pos/100).toFixed(2)}U</div>
                  </div>
                </div>
              </div>
            );
...
function AlertPanel({alerts, onDismiss}) {
  return(
    <div style={{width:300,minWidth:300,background:"#090d1a",borderLeft:"1px solid #1a2035",
      height:"100vh",overflowY:"auto",display:"flex",flexDirection:"column",boxSizing:"border-box",flexShrink:0}}>
      <div style={{padding:"12px 14px",borderBottom:"1px solid #1a2035",background:"#080c18",
        ...
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:7}}>
            ...
            {alerts.length>0&&<span style={{fontSize:10,color:"#ff9800",background:"#ff980022",
              padding:"1px 6px",borderRadius:10,fontWeight:700}}>{alerts.length}</span>}
          </div>
        </div>
        {alerts.length>0&&(
      ...
      <div style={{flex:1,overflowY:"auto",padding:"8px 10px"}}>
        {alerts.length===0?(
          <div style={{textAlign:"center",color:"#263238",marginTop:40,fontSize:11}}>
            ...
            <div style={{color:"#37474f"}}>等待訊號中…</div>
            <div style={{fontSize:9,color:"#1e293b",marginTop:6,lineHeight:1.7}}>
              掃描到進場/暴漲/暴跌<br/>訊號時會在這裡顯示
            </div>
          </div>
        ):alerts.map((a,i)=>{
          ...
          const isBearSetup= type==="bearSetup";
          const isEntry    = type==="entry";
          const bc = isCrash?"#ef5350":isSurge?"#ff9800":isBearSetup?"#ff7043":"#4caf50";
          const bg = isCrash?"rgba(239,83,80,0.07)":isSurge?"rgba(255,152,0,0.07)":
                     isBearSetup?"rgba(255,112,67,0.07)":"rgba(76,175,80,0.07)";
          const icon  = isCrash?"💥":isSurge?"🚀":isBearSetup?"⚠️":"🚀";
          const title = isCrash?"暴跌預警":isSurge?"暴漲預警":isBearSetup?"準備下跌":"進場訊號";
          const reasons=(isCrash||isBearSetup)?a.crash?.crashReasons:isSurge?a.surge?.surgeReasons:null;
          const dec = isEntry ? calcEntryDecision(a) : null;
          return(
            <div key={a.symbol+i+type} style={{background:bg,border:`1px solid ${bc}44`,
              borderLeft:`3px solid ${bc}`,borderRadius:7,padding:"10px 10px",marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                <div>
                  <div style={{fontSize:9,color:bc,letterSpacing:2,fontWeight:700,marginBottom:2}}>
                    {icon} {title}
                  </div>
                  <div style={{fontSize:17,fontWeight:800,color:"#e8eaf6",lineHeight:1}}>
                    {a.symbol.replace("USDT","")}
                    <span style={{fontSize:9,color:"#37474f",marginLeft:4}}>/USDT · {a.score}分</span>
                  </div>
                </div>
                <button onClick={()=>onDismiss(i)}
                  style={{background:"none",border:"none",color:"#37474f",cursor:"pointer",fontSize:14}}>✕</button>
              </div>
              ...
              {(isCrash||isBearSetup)&&a.price&&(
                <div style={{background:"#1a0505",borderRadius:5,padding:"6px 8px",
                  border:"1px solid #ef535033",marginBottom:6}}>
                  <div style={{fontSize:9,color:"#ff7043",marginBottom:3,fontWeight:700}}>▼ 放空參考</div>
                  <div style={{display:"flex",gap:8,fontSize:10,flexWrap:"wrap"}}>
                    <span>放空 <b style={{color:"#ff7043"}}>{a.price}</b></span>
                    <span>止損 <b style={{color:"#ef5350"}}>{a.ma45}</b></span>
                    <span>目標 <b style={{color:"#4caf50"}}>{a.ma60}</b></span>
                  </div>
                </div>
              )}

              {reasons&&reasons.slice(0,3).map((r,j)=>(
                <div key={j} style={{fontSize:9,color:"#546e7a",marginBottom:2,
                  display:"flex",gap:4,lineHeight:1.4}}>
                  <span style={{color:bc,minWidth:8,flexShrink:0}}>{(isCrash||isBearSetup)?"▼":"▲"}</span>
                  <span>{r}</span>
                </div>
              ))}

              <div style={{fontSize:8,color:"#1e293b",marginTop:5,display:"flex",justifyContent:"space-between"}}>
                <span>{new Date().toLocaleTimeString("zh-TW",{timeZone:"Asia/Taipei"})}</span>
                <span style={{color:bc,opacity:0.6}}>{type}</span>
              </div>
            </div>
          );
        })}
...
function App() {
  ...
  // Use refs so runScan always reads latest values without stale closure
  const intervalRef=useRef(interval), modeRef=useRef(mode), symsRef=useRef(customSyms);
  useEffect(()=>{intervalRef.current=interval;},[interval]);
  useEffect(()=>{modeRef.current=mode;},[mode]);
  useEffect(()=>{symsRef.current=customSyms;},[customSyms]);

  const runScan=useCallback(async(iv, scanMode, syms)=>{
    // Always read from refs to get latest values
    const _iv       = iv       ?? intervalRef.current;
    const _scanMode = scanMode ?? modeRef.current;
    const _syms     = syms     ?? symsRef.current;
    abortRef.current=false; setLoading(true); setError(null); setResults([]);
    try{
      let symList=_scanMode==="top100"?await getTopSymbols(100):
        _syms.split(/[,\s]+/).map(s=>s.trim().toUpperCase()).filter(Boolean);
      setProgress({done:0,total:symList.length});
      const newAlerts=[],out=[];
      for(let i=0;i<symList.length;i+=10){
        if(abortRef.current) break;
        const batch=symList.slice(i,i+10);
        const res=await Promise.all(batch.map(s=>analyseSymbol(s,_iv)));
        res.forEach(r=>{
          if(!r) return; out.push(r);
          const key=r.symbol+_iv;
          if(r.isEntry&&!seenAlerts.current.has(key+"e")){
            seenAlerts.current.add(key+"e");
            newAlerts.push({...r,alertType:"entry"});
            safeNotify(`🚀 進場：${r.symbol.replace("USDT","")}`, `評分${r.score} · ${_iv}`);
          }
          if(r.surge?.isSurge&&!seenAlerts.current.has(key+"s")){
            seenAlerts.current.add(key+"s");
            newAlerts.push({...r,alertType:"surge"});
            safeNotify(`🚀 暴漲：${r.symbol.replace("USDT","")}`, r.surge.surgeReasons[0]||"");
          }
          if(r.crash?.isCrash&&!seenAlerts.current.has(key+"c")){
            seenAlerts.current.add(key+"c");
            newAlerts.push({...r,alertType:"crash"});
            safeNotify(`💥 暴跌：${r.symbol.replace("USDT","")}`, r.crash.crashReasons[0]||"");
          }
          // 準備下跌預警
          if((r.crash?.isBearSetup||r.crash?.isCrash)&&!seenAlerts.current.has(key+"b")){
            if(r.crash?.isBearSetup&&!r.crash?.isCrash){
              seenAlerts.current.add(key+"b");
              newAlerts.push({...r,alertType:"bearSetup"});
              safeNotify(`⚠️ 準備下跌：${r.symbol.replace("USDT","")}`, r.crash.crashReasons[0]||"");
            }
          }
        });
      ...
      setLastScan(new Date().toLocaleTimeString("zh-TW",{timeZone:"Asia/Taipei"}));
  ...
  const filtered=results.filter(r=>{
    ...
    if(filter==="bearSetup") return r.crash?.isBearSetup&&!r.crash?.isCrash;
  ...
  const alertBadge = alerts.filter(a=>a.alertType==="crash"||a.alertType==="bearSetup").length;
  const entryBadge = alerts.filter(a=>a.alertType==="entry"||a.alertType==="surge").length;
    ...
    <div style={{display:"flex",height:"100vh",overflow:"hidden",
      ...
      {isMobile&&showSidebar&&(
        <div style={{position:"fixed",inset:0,zIndex:500,display:"flex"}}>
          <div style={{width:250,background:"#0b0f1c",borderRight:"1px solid #1a2035",
            height:"100vh",overflowY:"auto",padding:"14px 12px",boxSizing:"border-box",
            animation:"slideUp .25s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <span style={{fontSize:13,fontWeight:800,color:"#9fa8da"}}>設定</span>
              <button onClick={()=>setShowSidebar(false)}
                style={{background:"none",border:"none",color:"#546e7a",cursor:"pointer",fontSize:20}}>✕</button>
            </div>
            {/* Sidebar content rendered inline for mobile */}
            <div style={{fontSize:9,color:"#1e3a5f",letterSpacing:3,marginBottom:7}}>掃描模式</div>
            <button style={{...C.mB(mode==="top100"),marginBottom:6}} onClick={()=>{setMode("top100");setShowSidebar(false);runScan(undefined,"top100");}}>🏆 前100名</button>
            <button style={{...C.mB(mode==="custom"),marginBottom:6}} onClick={()=>{setMode("custom");setShowCustom(true);}}>✏️ 自訂幣種</button>
            {showCustom&&<textarea value={customSyms} onChange={e=>setCustomSyms(e.target.value)}
              style={{width:"100%",height:60,background:"#060810",border:"1px solid #1a2035",
                borderRadius:5,color:"#90a4ae",fontSize:10,padding:"5px",fontFamily:"inherit",resize:"vertical",marginBottom:6}}/>}
            <div style={{fontSize:9,color:"#1e3a5f",letterSpacing:3,marginBottom:7,marginTop:8}}>時間週期</div>
            {INTERVALS.map(iv=>(
              <button key={iv.value} style={{...C.ivB(interval===iv.value),marginBottom:4}}
                onClick={()=>{setIntervalVal(iv.value);runScan(iv.value);setShowSidebar(false);}}>
                {iv.label}
              </button>
            ))}
            <button onClick={()=>{runScan();setShowSidebar(false);}} disabled={loading}
              style={{...C.sBtn,marginTop:8}}>🔄 重新掃描</button>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:10}}>
              <span style={{fontSize:10,color:autoRefresh?"#4caf50":"#37474f"}}>
                {autoRefresh?"🔴 監控中":"自動監控"}
              </span>
      ...
      {isMobile&&showAlerts&&(
        <div style={{position:"fixed",inset:0,zIndex:500,display:"flex",flexDirection:"column"}}>
          <div style={{flex:1,background:"rgba(0,0,0,0.5)"}} onClick={()=>setShowAlerts(false)}/>
          <div style={{maxHeight:"80vh",overflowY:"auto",background:"#090d1a",
            borderTop:"1px solid #1a2035",animation:"slideUp .25s ease"}}>
      ...
      {/* ── 左側欄（桌面版）── */}
      <div className="desktop-only" style={C.sidebar}>
        <div>
            ...
            {results.length>0&&!loading&&(
              <div style={{background:"#060810",borderRadius:7,padding:"9px 11px",border:"1px solid #0f1629",fontSize:10}}>
                ...
                {[["掃描",progress.total||results.length,"#7986cb"],
                  ...
                  ["⚠️準備跌",results.filter(r=>r.crash?.isBearSetup&&!r.crash?.isCrash).length,"#ff7043"],
      ...
      {/* ── 主區 ── */}
      <div style={{...C.main, paddingBottom:isMobile?"70px":"12px"}}>
        {page==="oi" ? <OIDashboard/> :
        ...
        (<>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10,flexWrap:"wrap",gap:5}}>
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              {[["all","全部",null],["entry","🚀進場","#4caf50"],["surge","🚀暴漲","#ff9800"],
                ["crash","💥暴跌","#ef5350"],["bearSetup","⚠️準備跌","#ff7043"],
                ["smc","SMC多頭","#4dd0e1"],["strong","強訊號≥70",null],["above","站上三均",null]].map(([v,l,col])=>(
          ...
          {filtered.map((r,idx)=>(
            <div key={r.symbol} style={C.card(r.score,expanded===r.symbol,r.surge,r.crash)}>
              <div style={{display:"grid",
                gridTemplateColumns:isMobile?"40px 1fr 80px 56px":"24px 40px 1fr 86px 86px 86px 86px 58px 78px 56px",
                ...
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                    ...
                    {r.crash?.isBearSetup&&!r.crash?.isCrash&&<span style={{fontSize:8,color:"#ff7043",background:"#ff704322",padding:"1px 4px",borderRadius:3,fontWeight:700}}>⚠️準備跌</span>}
                    {r.maFan&&<span style={{fontSize:8,color:"#4dd0e1",background:"#00695c18",padding:"1px 4px",borderRadius:3}}>多↑</span>}
                  </div>
                </div>

                {(isMobile?[[r.price,"#90a4ae"]]:[[r.price,"#90a4ae"],[r.ma30,"#7986cb"],[r.ma45,"#9575cd"],[r.ma60,"#26c6da"]]).map(([v,c],i)=>(
                ...
                {!isMobile&&<span style={{fontSize:9,color:r.smc?.smcColor||"#37474f",
                ...
                <div style={{borderTop:"1px solid #0f1629",padding:"12px 14px"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
                    ...
                    <div>
                      ...
                      {/* 暴跌/準備下跌 詳情 */}
                      {(r.crash?.isCrash||r.crash?.isBearSetup)&&r.crash?.crashReasons?.length>0&&(
                        <div style={{marginTop:8,background:r.crash?.isCrash?"#1a0505":"#1a0d00",
                          borderRadius:7,padding:"9px 11px",
                          border:`1px solid ${r.crash?.isCrash?"#ef535044":"#ff704344"}`}}>
                          <div style={{fontSize:10,fontWeight:700,
                            color:r.crash?.isCrash?"#ef5350":"#ff7043",marginBottom:7}}>
                            {r.crash.crashStrength||"⚠️ 空頭訊號"}
                          </div>
                          {r.crash.crashReasons.map((rs,j)=>(
                            <div key={j} style={{fontSize:10,color:"#546e7a",marginBottom:4,lineHeight:1.4}}>
                              {rs}
                            </div>
                          ))}
                          {r.crash?.isBearSetup&&!r.crash?.isCrash&&(
                            <div style={{fontSize:9,color:"#ff7043",marginTop:5,
                              borderTop:"1px solid #37474f33",paddingTop:4,fontStyle:"italic"}}>
                              ⚠️ 準備下跌（分數{r.crash.crashScore}），建議觀察是否做空
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <div>
                      ...
                      {/* 做空機會提示 */}
                      {(r.crash?.isCrash||r.crash?.isBearSetup)&&(
                        <div style={{background:"#1a0a00",borderRadius:7,padding:"10px 12px",
                          border:"1px solid #ff704344",marginBottom:6,marginTop:4}}>
                          <div style={{fontSize:10,color:"#ff7043",fontWeight:700,marginBottom:6}}>
                            {r.crash?.isCrash?"💥 放空機會分析":"⚠️ 準備下跌 · 空頭觀察"}
                          </div>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                            {[
                              {l:"放空進場",v:r.price,c:"#ff7043"},
                              {l:"止損（上方）",v:r.smc?.bos?.price?(r.smc.bos.price*1.005).toFixed(4):"—",c:"#ef5350"},
                              {l:"目標1",v:r.ma45,c:"#8bc34a"},
                              {l:"目標2",v:r.ma60,c:"#4caf50"},
                            ].map(item=>(
                              <div key={item.l} style={{background:"#060810",borderRadius:4,padding:"5px 7px"}}>
                                <div style={{fontSize:8,color:"#37474f",marginBottom:1}}>{item.l}</div>
                          ...
                          <div style={{fontSize:9,color:"#546e7a",marginTop:6}}>
                            空頭條件：{r.crash?.crashReasons?.slice(0,2).join(" · ")}
                          </div>
                        </div>
                      )}
      ...
      <div className="mobile-bottom-bar" style={{
        ...
        background:"#080c18",borderTop:"1px solid #1a2035",
        display:"flex",alignItems:"center",justifyContent:"space-around",
        zIndex:400,boxShadow:"0 -4px 20px #00000088"
      }}>
        {[
          {icon:"📊",label:"掃描",p:"scanner"},
          {icon:"📈",label:"OI",p:"oi"},
          {icon:"📋",label:"日誌",p:"journal"},
        ].map(({icon,label,p})=>(
          <button key={p} onClick={()=>setPage(p)}
            style={{background:"none",border:"none",cursor:"pointer",
              display:"flex",flexDirection:"column",alignItems:"center",gap:2,
              color:page===p?"#9fa8da":"#37474f",fontFamily:"inherit",padding:"6px 14px",
              borderRadius:8,background:page===p?"#1a2040":"transparent"}}>
            <span style={{fontSize:20}}>{icon}</span>
            <span style={{fontSize:9,fontWeight:page===p?700:400}}>{label}</span>
          </button>
        ))}
        {/* 設定按鈕（開側欄） */}
        <button onClick={()=>setShowSidebar(true)}
          style={{background:"none",border:"none",cursor:"pointer",
            display:"flex",flexDirection:"column",alignItems:"center",gap:2,
            color:"#37474f",fontFamily:"inherit",padding:"6px 14px"}}>
          <span style={{fontSize:20}}>⚙️</span>
          <span style={{fontSize:9}}>{interval}</span>
        </button>
        {/* 通知按鈕 */}
        <button onClick={()=>setShowAlerts(true)}
          ...
          {alerts.length>0&&(
            <span style={{position:"absolute",top:2,right:8,minWidth:16,height:16,
              background:"#ef5350",borderRadius:8,fontSize:9,color:"#fff",
              display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>
              {alerts.length}
            </span>
          )}
          <span style={{fontSize:9}}>通知</span>
        </button>
      </div>
    </div>
  );
